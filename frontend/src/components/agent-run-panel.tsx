...
function CodeBlock({ label, content, tone }: { label: string; content: string; tone: "rose" | "emerald" }) {
  return (
    <div>
      <p className={tone === "rose" ? "mb-1 font-mono text-[9px] uppercase tracking-[0.14em] text-rose-600" : "mb-1 font-mono text-[9px] uppercase tracking-[0.14em] text-emerald-600"}>
        {label}
      </p>
      <pre className="max-h-52 overflow-auto whitespace-pre-wrap break-words rounded-lg bg-black/[0.04] p-2 font-mono text-[10px] leading-4 text-foreground/75 dark:bg-white/[0.05]">
        {content}
      </pre>
    </div>
  );
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-[var(--mn-line)] bg-[var(--mn-surface)]">
      ...
      >
        ...
        {hasSnippet && (open ? <ChevronDown className="size-3.5" /> : <ChevronRight className="size-3.5" />)}
      </button>
      {open && hasSnippet && (
        <div className="space-y-2 border-t border-[var(--mn-line)] px-3 pb-3 pt-2">
          {item.beforeSnippet && <CodeBlock label="Removed" content={item.beforeSnippet} tone="rose" />}
          {item.afterSnippet && <CodeBlock label="Added" content={item.afterSnippet} tone="emerald" />}
        </div>
      )}
    </div>
  );
