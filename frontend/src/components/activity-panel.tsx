...
export function ActivityPanel({ activities, workspace, running }: ActivityPanelProps) {
  return (
    <aside className="hidden w-[280px] shrink-0 flex-col border-l border-[var(--mn-line)] bg-[var(--mn-surface-muted)] xl:flex">
      <div className="flex h-16 items-center justify-between border-b border-[var(--mn-line)] px-5">
        <div>
          <p className="text-xs font-semibold">Agent activity</p>
          <p className="mt-0.5 text-[0.6875rem] text-muted-foreground">Live execution telemetry</p>
        </div>
        <Badge
          variant="outline"
          className="border-[var(--mn-line)] bg-[var(--mn-surface)] text-[0.6875rem]"
        >
          {running ? "streaming" : "idle"}
        </Badge>
      </div>
      <div className="flex-1 space-y-4 overflow-y-auto p-4">
        <Card className="mn-surface py-4 shadow-none">
          <CardHeader className="px-4 pb-2">
            <CardTitle className="flex items-center gap-2 text-xs font-medium">
              <Cpu className="size-3.5 text-[var(--mn-accent-strong)]" />
              Session health
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-3 px-4">
            <Metric
              label="Workspace"
              value={workspace.ready ? "Indexed" : "Waiting"}
              icon={workspace.ready ? CheckCircle2 : Clock3}
              tone="text-emerald-600 dark:text-emerald-200"
            />
            <Metric
              label="Permission mode"
              value="Ask before tools"
              icon={ShieldCheck}
              tone="text-[var(--mn-accent-strong)]"
            />
            <Metric
        ...
        <div className="px-1">
          <div className="mb-3 flex items-center justify-between">
            ...
            <span className="font-mono text-[0.6875rem] text-slate-600">
          ...
          {activities.length === 0 ? (
            <div className="mn-dot-grid rounded-lg border border-dashed border-white/[0.1] px-4 py-7 text-center">
              ...
              <p className="mt-1 text-[0.6875rem] leading-relaxed text-slate-600">
                Tool calls, subagents and file changes will appear here.
              </p>
            </div>
          ) : (
...
function Metric({
  ...
  return (
    <div className="flex items-center gap-2.5">
      ...
      <span className="ml-auto text-[0.6875rem] font-medium">{value}</span>
    </div>
  );
}
...
function ActivityRow({ item }: { item: ActivityItem }) {
  ...
  return (
    <div className="flex gap-3 rounded-lg border border-[var(--mn-line)] bg-[var(--mn-surface)] p-3">
      <div className="mt-0.5">
        {item.active ? (
          <Loader2 className="size-3.5 animate-spin text-[var(--mn-accent-strong)]" />
        ) : (
          <Icon className="size-3.5 text-muted-foreground" />
        )}
      </div>
      <div className="min-w-0">
        <p className="truncate text-[0.75rem] font-medium">{item.label}</p>
