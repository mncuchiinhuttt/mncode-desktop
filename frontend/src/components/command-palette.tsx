    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="top-[18%] translate-y-0 overflow-hidden border-[var(--mn-line)] bg-[var(--mn-surface)] p-0 text-foreground shadow-2xl sm:max-w-xl">
        <DialogHeader className="border-b border-[var(--mn-line)] px-4 py-3">
          ...
          <DialogDescription className="sr-only">Search desktop actions</DialogDescription>
        </DialogHeader>
        <div className="p-3">
          ...
          <div className="mt-3 space-y-1">
            ...
            <CommandItem
              ...
              shortcut="W"
              onClick={() => run(() => onNavigate("workspace"))}
            />
            <CommandItem
              icon={FileSearch}
              label="Inspect codebase"
              shortcut="I"
              onClick={() => run(() => onNavigate("insights"))}
            />
            <CommandItem
              icon={Settings2}
              label="Open settings"
              shortcut="S"
              onClick={() => run(() => onNavigate("settings"))}
            />
            <CommandItem
              icon={Sparkles}
              label="Open Skills Marketplace"
              shortcut="M"
              onClick={() => run(() => onNavigate("skills"))}
            />
          </div>
...
function CommandItem({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="group flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left text-xs text-foreground/75 transition-colors hover:bg-[var(--mn-surface-muted)] hover:text-foreground"
    >
      <Icon className="size-4 text-muted-foreground group-hover:text-[var(--mn-accent-strong)]" />
      <span>{label}</span>
